...
if __name__ == "__main__":
    ...
    for dataset, dataset_idx in datasets:
        ...
        for pl in paranoia_levels:
            ...
            adv_ms_loader = DataLoader(
                malicious_path=ms_adv_pl_path % pl, legitimate_path=legitimate_test_path
            )
            ...
            for model_name in other_models:
                ...
                if model_name == "modsec":
                    ...
                    waf = PyModSecurity(rules_dir=crs_dir, pl=pl)
            ...
            for model_name in models:
                ...
                for penalty in penalties:
                    if model_name == "svc":
                        ...
                        model = joblib.load(
                            os.path.join(
                                models_path, f"linear_svc_pl{pl}_{penalty}.joblib"
                            )
                        )
                        y_scores = model.decision_function(xts)

                        if penalty == "l1":
                            adv_y_scores = model.decision_function(
                                adv_svm_linear_l1_xts
                            )
                            adv_yts = adv_svm_linear_l1_yts
                        else:
                            adv_y_scores = model.decision_function(
                                adv_svm_linear_l2_xts
                            )
                        ...
                        model = joblib.load(
                            os.path.join(
                                models_path, f"log_reg_pl{pl}_{penalty}.joblib"
                            )
                        )
                        y_scores = model.predict_proba(xts)[:, 1]
    ...
    fig.text(0.5, 0.07, "False Positive Rate (FPR)", ha="center", va="center", fontsize=16)
    fig.text(0.098, 0.5, "True Positive Rate (TPR)", ha="center", va="center",
         rotation="vertical", fontsize=16)
    fig.savefig(
        os.path.join(figures_path, "roc_curves_training.pdf"),
        dpi=600,
        format="pdf",
        bbox_inches="tight",
    )
