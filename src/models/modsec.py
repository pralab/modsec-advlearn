...
class PyModSecurity:
    ...
    def __init__(
        self, rules_dir, threshold=5.0, pl=4, output_type="score", debug=False
    ):
        """
        ...
        if not 1 <= pl <= 4:
            raise ValueError(
                "Invalid value for pl input param: {}. Valid values are: [1, 2, 3, 4]".format(
                    pl
                )
            )
        ...
        if output_type not in ["binary", "score"]:
            raise ValueError(
                "Invalid value for mode input param: {}. Valid values are: ['binary', 'score']".format(
                    output_type
                )
            )
        ...
        if self._debug:
            print(
                "[INFO] Using ModSecurity CRS with PL = {} and INBOUND THRESHOLD = {}".format(
                    pl, threshold
                )
            )
    ...
    def _process_query(self, payload: str):
        ...
        rules_logger_cb = RulesLogger(threshold=self._threshold, debug=self._debug)
        ...
        transaction.processURI(
            "http://127.0.0.1/test?{}".format(payload), "GET", "HTTP/1.1"
        )
    ...
    def predict(self, X):
        ...
        if isinstance(X, list) or len(X.shape) == 1:
            ...
            raise ValueError(
                "Invalid input shape. Expected 1D array or list, got {}D array".format(
                    len(X.shape)
                )
            )
...
class RulesLogger:
    ...
    def __init__(self, threshold=5.0, regex_rules_filter=None, debug=False):
        ...
        self._debug = debug
        self._rules_filter = (
            re.compile(regex_rules_filter)
            if regex_rules_filter is not None
            else re.compile("^.*")
        )
        self._score = 0.0
        self._threshold = threshold
    ...
    def __call__(self, data, rule_message):
        ...
        if self._debug:
            ...
            print(
                "[DEBUG] ID: {}, Message: {}, Phase: {}, Severity: {}".format(
